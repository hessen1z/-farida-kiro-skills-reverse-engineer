...
## Overview
Digital signatures provide integrity and origin authentication for data and code.

## Patterns
- Verify the signature with the signers public key and explicit algorithm.
- Check that the signature uses a strong hash and padding mode.
- Validate the full certificate chain when the public key comes from a certificate.
- Treat signature failure as a critical error and do not proceed with untrusted data.

## Practical Workflow
1. Parse signed data and separate the signature block.
2. Identify the algorithm and hash used for signing.
3. Verify the signature against the expected public key.
4. Confirm certificate or key validity if used.
