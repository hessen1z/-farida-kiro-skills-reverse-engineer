...
## Prompt
You are a security analyst reviewing a network capture for encrypted transport usage and protocol validation. Explain whether secure channels are configured correctly, whether handshake and certificate validation are complete, and whether any fallback or downgrade behavior is present.

## Context
- Focus on TLS/HTTPS, VPN tunnels, or custom encryption layers.
- Include certificate chain and cipher suite correctness.
- Note any plaintext credentials or insecure fallback to unencrypted protocols.

## Output
- Security assessment of the transport.
- List of protocol mismatches or insecure behaviors.
- Recommended validation steps and remediation.
